...
app.post('/scan', async (req, res) => {
  ...
  // Generate QR Code Data URL dari qr_code yang discan
  let qrCodeDataURLScan;
  try {
    qrCodeDataURLScan = await QRCode.toDataURL(qr_code); // Generate QR Code image dari data yang discan
  } catch (error) {
    console.error('Error generating QR Code image from scan data:', error);
    qrCodeDataURLScan = null; // Handle jika gagal generate QR Code image
  }
  ...
  if (qrData && qrData.nama_laporan && qrData.lokasi && qrData.tanggal && qrData.deskripsi) {
    ...
    values = [qrData.nama_laporan, qrData.lokasi, qrData.tanggal, qrData.deskripsi, qrCodeDataURLScan]; // Gunakan qrCodeDataURLScan disini
  } else {
    // Jika QR tidak sesuai format, masukkan ke deskripsi dengan kategori 'Other QR'
    query = 'INSERT INTO laporan_scan (nama_laporan, lokasi, tanggal, deskripsi, qr_code) VALUES (?, ?, ?, ?, ?)';
    values = ['Other QR', 'Other QR', 'Other QR', qr_code, qrCodeDataURLScan]; // Gunakan qrCodeDataURLScan dan qr_code disini
  }
...
// Endpoint baru untuk mendapatkan data laporan berdasarkan ID dan jenis laporan
app.get('/api/laporan/:jenis_laporan/:id', (req, res) => {
  const jenisLaporan = req.params.jenis_laporan; // 'create' atau 'scan'
  const id = req.params.id;
  let tabelLaporan = '';
  let kolomId = '';

  if (jenisLaporan === 'create') {
    tabelLaporan = 'laporan_create';
    kolomId = 'id_create';
  } else if (jenisLaporan === 'scan') {
    tabelLaporan = 'laporan_scan';
    kolomId = 'id_scan';
  } else {
    return res.status(400).json({ error: 'Jenis laporan tidak valid' });
  }

  const query = `SELECT * FROM ${tabelLaporan} WHERE ${kolomId} = ?`;

  db.query(query, [id], async (err, results) => {
    if (err) {
      console.error(`Error saat mengambil data laporan ${jenisLaporan} dengan ID ${id}:`, err);
      return res.status(500).json({ error: `Gagal mengambil data laporan ${jenisLaporan}` });
    }
    if (results.length === 0) {
      return res.status(404).json({ message: `Laporan ${jenisLaporan} tidak ditemukan dengan ID ${id}` });
    }
    res.json(results[0]); // Mengirimkan data laporan secara spesifik
  });
});
